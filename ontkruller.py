...
import sys
import math
import levelherkenner


g = open("frequentie.txt", 'w')
f = open("ontkruller.txt", 'w')
...
leveltype = levelherkenner.levelherkenner(level, level_breedte, level_hoogte)
...
permanent_doodlopende_coordinaten = permanent_doodloopcheck_level(j, k, level_breedte, level_hoogte, level, f)
...
def kortstepadzoekerhulp(x1, y1, x2, y2, horizontaal, verticaal):
    ...
    #         break
    u = 0
    while u < min(100, (math.factorial(abs(x1 - x2) + abs(y1 - y2)))/(math.factorial(abs(x1 - x2))*math.factorial(abs(y1 - y2)))):
        pad = permuteer(abs(x1 - x2), abs(y1 - y2), 'd', 'e', x0, y0, horizontaal, verticaal)
        if pad.count('.') + pad.count('x') == len(pad):
            f.write("\n")
            f.write("Er is een pad en het is een dit: " + str(pad))
            return pad
        else:
            u += 1
    
    f.write("\n")
    f.write("er zijn gewoon geen paden")
    return 'geen directe paden'
...
def muur_check(x,y):
    ...
    if level[y][x] != "." and level[y][x] != "x" and [x,y] in achterwerken:
        ...
        return -10000
    elif [x,y] in momenteel_doodloopcheck_level(level_breedte, level_hoogte, level, spelercoordinaten[tijdstap][speler_nummer][0], f) and lengtes[speler_nummer] > 1:
        return -1000
    else:
        return 0
...
def potentieel_doodlopend_check(x, y):
    if (levelcheck([x,y]) == '.' or levelcheck([x,y]) == 'x') and lengtes[speler_nummer] > 1:
        ...
        if areasigns.count('.') + areasigns.count('x') + areasigns.count(str(speler_nummer)) == 2:
            ...
            if len(al_gehad) > 0:
                i = 0
                while i < len(al_gehad):
                    al_gehad.remove(al_gehad[i])
                    i += 1
            potentieel_doodlopende_vakjes = potentieel_doodlopend(level_hoogte, level_breedte, level, levelcheck, Area, speler_nummer)
        
            pot = potentieel_doodlopende_gang_check(x, y, spelercoordinaten[tijdstap][speler_nummer][0][0], spelercoordinaten[tijdstap][speler_nummer][0][1], level, levelcheck, al_gehad, speler_nummer, afstand, kortstepadzoeker, slanghoofden, level_hoogte, level_breedte, Area, potentieel_doodlopende_vakjes, f)
            f.write("\n")
            f.write("De doodlopende gang is " + str(al_gehad))
            f.flush()
            if pot == 'geen gevaar':
                return 0
            elif len(al_gehad) + 3 >= pot:
                f.write("\n")
                f.write("\n")
                f.write("HELPIEPELPIE!!!!!")
                f.flush()
                return -100
            else:
                return -2
            ...
            f.write("\n")
            f.write("veilig wat betreft potentnieeel doodlopenede gangen")
            f.flush()
            return 0
    else:
        f.write("\n")
...
while True:
    ...
    if tijdstap == 10:
        ...
        if leveltype == 1 and eten_frequentie < 0.05:
            f.write("\n")
            f.write("eetmodus geactiveerd!")
            f.flush()
            eetmodus = 1
        ...
        f.write("\n")
        f.flush()
        g.write("De frequentie van het eten is ongeveer " + str(eten_frequentie) + " per beurt per speler per honderd vakjes")
        g.flush()
