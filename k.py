...
def kortstepadzoeker(x1, y1, x2, y2):
    ...
    j = 0
    while j < (math.factorial(abs(x1 - x2) + abs(y1 - y2)))/(math.factorial(abs(x1 - x2))*math.factorial(abs(y1 - y2))):
    ...
    veilige_paden = copy.deepcopy(paden)
    j = 0
    while j < len(paden):
        k = 0
        while k < len(paden[j]):
            if level[paden[j][k][1]][paden[j][k][0]] != '.':
                if level[paden[j][k][1]][paden[j][k][0]] != 'x':
                    veilige_paden[j] = 0
                    j += 1
            k += 1
        j += 1
    
    veilige_paden = list(set(veilige_paden))
    veilige_paden.remove(0)
...
def vrije_coordinaten_check(x,y):
    ...
    if omsingel != []:
        ...
        while j < len(omsingel):
            omsingel_scores.append(scores[omsingel[j]])
            j += 1
...
def kortstepad_check(x, y):
    if kortstepad != 'geen directe paden':
        if [x,y] == kortstepad[1] and eetmodus == 1:
            return 100
    else:
        return 0
...
def beste_zet(x, y):
    ...
    for i in range(0,len(area)):
        ...
        Mark = muur_check(area[i][0], area[i][1]) + eten_check(area[i][0], area[i][1]) + vrije_coordinaten_check(area[i][0], area[i][1]) + X + kortstepad_check(area[i][0], area[i][1])
        rating.append(Mark)
...
while True:
    ...
    # zet alle plekken met eten in een lijstje
    etenscoordinaten = []
    j = 0
    while j < level_hoogte:
        k = 0
    ...
    # bereken de afstanden tot het eten
    etensafstanden = []
    ...
    if len(etensafstanden) > 0:
        j = etensafstanden.index(min(etensafstanden))
        kortpad = kortstepadzoeker(spelercoordinaten[tijdstap][speler_nummer][0][0], spelercoordinaten[tijdstap][speler_nummer][0][1], etenscoordinaten[j][0], etenscoordinaten[j][1])
        while kortpad == 'geen directe paden':
